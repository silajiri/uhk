...
*   **Pedagogický cíl (Integrovaná reflexe):** Hra má dvě hlavní fáze – Akci (anonymní spolupráce ve 4 úrovních) a Zpětný pohled (interaktivní odmaskování a společný real-time chat). Cílem je nejprve vytvořit anonymní zkušenost a poté vést žáky k tomu, aby na ni aplikovali svůj vlastní úsudek a očekávání.
*   **Bezpečí a podpora:** V závěrečné reflexi je kladen důraz na psychologické bezpečí třídy a na vyzdvihnutí vzájemné podpory žáků.

## 2. Herní úrovně a struktura
Hra se skládá ze čtyř hlavních úrovní a závěrečné reflexní fáze:
...
3.  **Level 3: Skleněný most** (Paměťový přechod mostu o velikosti N x N, aktivní hráč vyhledává pochozí dlaždice, sledující parťák posílá průběžné reakce a na konci povinně volí finální hodnocení 👏/😂; při 3 chybách se role střídají).
4.  **Level 4: Kód pravdy** (Skládání 5místného kódu ze dvou oddělených úlomků, které si hráči musí zaslat. Při 3 chybách se kód změní).
5.  **Závěrečná reflexe** (Po schválení učitelem se zobrazí statistiky, otočením karty odhalí skutečné jméno parťáka, zdůrazní se bezpečí ve třídě a odemkne se volný real-time chat).
